...
impl Png {
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        let chunk_bytes = self.chunks.iter().flat_map(|c| c.as_bytes());
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(mut value: &[u8]) -> Result<Self, Self::Error> {
        ...
        loop {
            let ref_reader = value.by_ref();
            let mut length_buf = [0_u8; 4];

            if ref_reader.read_exact(&mut length_buf).is_err() {
                break;
            }

            let length = u32::from_be_bytes(length_buf);

            let mut chunk_bytes = length_buf.to_vec();
